...
TRACY_DEBUG :: #config(TRACY_DEBUG, false)

TYPE :: "debug" when TRACY_DEBUG else "release"

when ODIN_OS == .Darwin {
	#panic("TODO: Support tracy on MacOS")
}
when ODIN_OS == .Windows {
	#panic("TODO: Support tracy on Windows")
}
when ODIN_OS == .Linux {
	// NOTE: I couldn't get the tracy 0.12.2 service working on NixOS so I am using 0.11.1 libs for now
	LIB_TRACY_CLIENT :: "./lib/0.11.1/libTracyClient-" + TYPE + ".a"
	// NOTE: This expects tracy to be built with clang so it links against `stdc++` and not `c++`
	foreign import tracy {LIB_TRACY_CLIENT, "system:stdc++"}
}
