...
GLFW_DEBUG :: #config(GLFW_DEBUG, false)
...
when ODIN_OS == .Windows {
	...
	when GLFW_SHARED {
		foreign import glfw {"system:glfw", "system:Cocoa.framework", "system:IOKit.framework", "system:OpenGL.framework"}
	} else {
		foreign import glfw {"../lib/darwin/libglfw3.a", "system:Cocoa.framework", "system:IOKit.framework", "system:OpenGL.framework"}
	}
} else {
	when GLFW_SHARED {
		...
		@(private)
		TYPE :: "debug" when GLFW_DEBUG else "release"
		@(private)
		LIBGLFW3 :: "../lib/libglfw3-" + TYPE + ".a"
		when !#exists(LIBGLFW3) {
			#panic(
				"Could not find the static glfw library, add it at \"" +
				ODIN_ROOT +
				"vendor/glfw/lib/\"`",
			)
		}
